...
def generate_launch_description():
    ...
    launch_dir = os.path.join(get_package_share_directory('openrover_demo'), 'launch')
    ...
    declare_world_cmd = launch.actions.DeclareLaunchArgument(
        'world',
        default_value='worlds/willowgarage.world',
    ...
    presence_yaml = os.path.join(get_package_share_directory('openrover_demo'), 'config', 'presence.yaml')

    # Specify the actions
    start_gazebo_cmd = launch.actions.ExecuteProcess(
        condition=IfCondition(use_simulation),
        cmd=[simulator, '-s', 'libgazebo_ros_init.so', '-s', 'libgazebo_ros_factory.so', world, '__params:=/home/cottsay/demo_ws/use_sim_time.yaml'],
    ...
    start_robot_state_publisher_cmd = launch.actions.ExecuteProcess(
        cmd=[
            ...
            os.path.join(
                get_package_share_directory('openrover_demo'),
                'urdf', 'rover.urdf'),
            ['__params:=', params_file]],
        cwd=[launch_dir], output='screen')

    start_se_node_cmd = launch.actions.ExecuteProcess(
        cmd=[
            os.path.join(
                get_package_prefix('robot_localization'),
                'lib/robot_localization/se_node'),
            ['__params:=', presence_yaml],
            'odometry/filtered:=odom'],
        cwd=[launch_dir], output='screen')

    start_urdf_spawner_cmd = launch.actions.ExecuteProcess(
        cmd=[
            os.path.join(
                get_package_prefix('openrover_demo'),
                'lib/openrover_demo/urdf_spawner.py'),
            os.path.join(
                get_package_share_directory('openrover_demo'),
                'urdf', 'rover.urdf'),
            ],
    ...
    ld.add_action(start_se_node_cmd)
    ld.add_action(start_urdf_spawner_cmd)
